#ifndef H5_CHECK_DEFINES_H__
#define H5_CHECK_DEFINES_H__
...
#define SUPERBLOCK_VARLEN_SIZE(v)                    \
    ( (v == 0 ? SUPERBLOCK_VARLEN_SIZE_V0 : 0)                   \
    + (v == 1 ? SUPERBLOCK_VARLEN_SIZE_V1 : 0)                   \
    + (v == 2 ? SUPERBLOCK_VARLEN_SIZE_V2 : 0))
...
#define SUPERBLOCK_REMAIN_SIZE(v, fs)                             \
    ( (v == 0 ? SUPERBLOCK_REMAIN_SIZE_V0(fs) : 0)              \
    + (v == 1 ? SUPERBLOCK_REMAIN_SIZE_V1(fs) : 0)               \
...
struct B2_class_t {
    ...
    ck_err_t (*decode)(driver_t *f, const uint8_t *raw, void *record); 
...
typedef struct bt2_shared_t { /* shared info */
...
typedef struct B2_node_ptr_t { /* node specific info */
    ck_addr_t     addr;
    unsigned      node_nrec;
    ck_hsize_t       all_nrec;  
} B2_node_ptr_t;
...
#define HF_MAN_INDIRECT_CHILD_DIR_ENTRY_SIZE(fs, h) (                           \
    ((h)->filter_len > 0 ?                                                    \
        ((fs)->size_offsets + (fs)->size_lengths + 4) : /* Size of entries for filtered direct blocks */ \
        (fs)->size_offsets)             /* Size of entries for un-filtered direct blocks */ \
    )


/* Size of managed indirect block */
#define HF_MAN_INDIRECT_SIZE(fs, h, i) (                                        \
    /* General metadata fields */                                             \
    HF_METADATA_PREFIX_SIZE(TRUE)                                           \
                                                                              \
...
#define HF_MAN_ABS_DIRECT_OVERHEAD(fs, h) (                                     \
    /* General metadata fields */                                             \
